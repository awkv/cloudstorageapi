...
#include "cloudstorageapi/internal/type_list.h"
#include <any>
#include <set>
#include <typeindex>
#include <typeinfo>
...
void CheckExpectedOptionsImpl(std::set<std::type_index> const&, Options const&, char const*);
}  // namespace internal
 ...
 * convention they are named like "FooOption". Each class or library
 * may define their own set of options. Additionally, there are common
...
public:
    ...
    {
        m_map[typeid(T)] = Data<T>{std::move(v)};
        return *this;
    }
    ...
    {
        static auto const* const DefaultValue = new ValueTypeT<T>{};
        auto const it = m_map.find(typeid(T));
        if (it != m_map.end())
            return std::any_cast<Data<T>>(&it->second)->value;
        return *DefaultValue;
    }
    ...
    {
        auto const p = m_map.emplace(typeid(T), Data<T>{std::move(init_value)});
        return std::any_cast<Data<T>>(&p.first->second)->value;
    }

private:
    friend Options internal::MergeOptions(Options, Options);
    friend void internal::CheckExpectedOptionsImpl(std::set<std::type_index> const&, Options const&, char const*);
    ...
    // The `std::any` objects all hold a `Data<T>`
...
template <typename... T>
void CheckExpectedOptionsImpl(OptionList<T...> const&, Options const& opts, char const* caller)
{
