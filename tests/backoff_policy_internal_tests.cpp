...
using csa::internal::ExponentialBackoffPolicy;
...
{
    EXPECT_THROW(ExponentialBackoffPolicy(ms(10), ms(50), 0.0), std::invalid_argument);
    EXPECT_THROW(ExponentialBackoffPolicy(ms(10), ms(50), 1.0), std::invalid_argument);
}
...
TEST(ExponentialBackoffPolicy, ClonesHaveDifferentSequences)
{
    ...
    std::generate_n(sequence_1.begin(), test_length, [&] { return c1->OnCompletion().count(); });

    std::vector<milliseconds_type> sequence_2(test_length);
    std::generate_n(sequence_2.begin(), test_length, [&] { return c2->OnCompletion().count(); });
