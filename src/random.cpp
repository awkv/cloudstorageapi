...
std::vector<unsigned int> FetchEntropy(std::size_t desiredBits)
{
    ...
    // One would like to use `rdrand` as the token, but:
    //   - `rdrand` is not supported by all versions of libstdc++
    //   - even for the versions that do support it, that is CPU specific
    //   - there is no reliable way to detect if the library version supports
    ...
    auto constexpr WordSize = std::numeric_limits<unsigned int>::digits;
    auto const n = (desiredBits + WordSize - 1) / WordSize;
