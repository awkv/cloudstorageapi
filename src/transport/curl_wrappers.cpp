...
namespace internal {
namespace {
// The cloustorageapi library depends on libcurl, which depends
...
//
std::once_flag SslLockingInitialized;
...
#define CSA_SSL_REQUIRES_LOCKS 1
#else
#define CSA_SSL_REQUIRES_LOCKS 0
#endif

#if CSA_SSL_REQUIRES_LOCKS
std::vector<std::mutex> SslLocks;
...
void InitializeSslLocking(bool enableSslCallbacks)
{
    std::string curlSsl = CurlSslLibraryId();
    ...
    // version.
    if (expectedPrefix.rfind("LibreSSL 2", 0) == 0)
    {
        expectedPrefix = "LibreSSL 2";
    }
#ifdef OPENSSL_VERSION
    std::string opensslVer = OpenSSL_version(OPENSSL_VERSION);
#else
    std::string opensslVer = SSLeay_version(SSLEAY_VERSION);
    ...
    if (opensslVer.rfind(expectedPrefix, 0) != 0)
    {
    ...
    //
    CSA_LOG_INFO("Installing SSL locking callbacks.");
    SslLocks = std::vector<std::mutex>(static_cast<std::size_t>(CRYPTO_num_locks()));
    ...
    // C functions from C++. In my opinion, we can rely on the
    // default version.
}
#else
