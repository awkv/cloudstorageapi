...
StatusOrVal<RefreshingCredentialsWrapper::TemporaryToken>
GoogleAuthHandler::ParseOAuth2RefreshResponse(internal::HttpResponse const& response,
    std::chrono::system_clock::time_point now)
{
    ...
    if (refreshJson.is_discarded() || refreshJson.count("access_token") == 0 ||
        refreshJson.count("expires_in") == 0 ||
        refreshJson.count("token_type") == 0)
    {
        ...
        return AsStatus(internal::HttpResponse{ response.m_statusCode,
                                                payload, response.m_headers });
    }
    ...
    auto expiresIn =
        std::chrono::seconds(refreshJson.value("expires_in", int(0)));
    auto newExpiration = now + expiresIn;
    return RefreshingCredentialsWrapper::TemporaryToken{ std::move(header),
                                                        newExpiration };
}

StatusOrVal<OAuth2CredentialsInfo> GoogleAuthHandler::ParseOAuth2Credentials(
    internal::nl::json const& jsonCreds, std::string const& source)
{
    ...
    for (auto const& key :
        { clientIdKey, clientSecretKey, refreshTokenKey })
    {
        if (jsonCreds.count(key) == 0)
        {
            return Status(StatusCode::InvalidArgument,
                "Invalid OAuth2Credentials, the " +
                std::string(key) +
    ...
    return OAuth2CredentialsInfo{
      jsonCreds.value(clientIdKey, ""),
      jsonCreds.value(clientSecretKey, ""),
